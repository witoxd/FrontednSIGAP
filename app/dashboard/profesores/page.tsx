      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        ...
        {(isLoading && !isSearchMode) ? (
          ...
          displayData.map((pro, i) => {
            ...
            return (
              <div key={pro.profesor.profesor_id} className="border-b border-border/60">
                {/* ── Fila desktop ── */}
                <div
                ...
                >
                  ...
                  <span className="text-[11px] text-muted-foreground truncate">{pro.docente.area ?? "—"}</span>
                  <div className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-semibold w-fit ${cls.badge}`}>
                    <div className={`w-1.5 h-1.5 rounded-full ${cls.dot}`} />{lbl}
                  </div>
                  {/* Acciones desktop */}
                ...
                <div className={`flex md:hidden items-start gap-3 px-4 py-3 transition-colors ${i % 2 === 0 ? "bg-transparent" : "bg-muted/20"}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-foreground leading-tight">
                      {pro.persona.apellido_paterno ?? ""}{pro.persona.apellido_materno ? ` ${pro.persona.apellido_materno}` : ""}, {pro.persona.nombres ?? ""}
                    </p>
                    ...
                    <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                      ...
                      {pro.docente.area && (
                        <span className="text-[10px] text-muted-foreground">{pro.docente.area}</span>
                      )}
                    </div>
                  </div>
