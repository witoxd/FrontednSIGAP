  ...
  return (
    <div className="flex flex-col gap-0">
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        ...
        <div className="hidden md:grid bg-muted/40 border-b border-border px-6 py-2 gap-0"
        ...
        {(isLoading && !isSearchMode) ? (
          ...
          displayData.map((est, i) => {
            ...
            return (
              <div key={est.estudiante.estudiante_id} className="border-b border-border/60">
                {/* ── Fila desktop ── */}
                <div
                ...
                <div className={`flex md:hidden items-start gap-3 px-4 py-3 transition-colors ${i % 2 === 0 ? "bg-transparent" : "bg-muted/20"}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-foreground leading-tight">
                      {est.persona.apellido_paterno ?? ""}{est.persona.apellido_materno ? ` ${est.persona.apellido_materno}` : ""}, {est.persona.nombres ?? ""}
                    </p>
                    ...
                    <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                      ...
                      {fechaIngreso !== "—" && (
                        <span className="text-[10px] text-muted-foreground/60">{fechaIngreso}</span>
                      )}
                    </div>
                  </div>
